...
class LoginProcessor(BaseProcessor):
    def process(self):
        ...
        if not re.match(r'^[a-zA-Z]{3,12}$', request):
            return False
        character_name = self._client_connection.character.name = request.capitalize()
        LOGGER.debug(
            '%s:%s - %s connected' % (
        ...
        self._client_connection.send('Welcome %s!' % character_name)
        matching_client_connections = ClientConnectionManager.instance.get_all_except(
            [self._client_connection], exclude_waiting_for_name=True)
        for matching_client_connection in matching_client_connections:
            matching_client_connection.send('%s just arrived' % character_name,
